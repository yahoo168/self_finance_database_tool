...
class Database(object):
    ...
    def _save_stockPrice(self, ticker, folder_name, country, data_source):
        # Documentation1: https://pypi.org/project/yfinance/
        # Documentation2: https://aroussi.com/post/python-yahoo-finance
        if data_source == "yfinance":
            ...
            # yfinance預設會額外下載dividend與stock-split資料，actions設為False以改變設定
            price_data = yf.Ticker(ticker_for_search).history(period="max", actions=False)
            # yfinance預設為首字母大寫，此處調整會全小寫
            price_data.columns = [x.lower() for x in price_data.columns]
            # yfinance預設會自動調整OHLC，故不存在adjclose欄位，此處額外補上
            price_data.index = price_data.index.tz_localize(None)
            # print(price_data)
            # print(price_data.index)
            price_data["adjclose"] = price_data["close"]
            price_data.to_pickle(file_position)


        ## yahoo_fin停止更新（最新版為2021-07）
        # Documentation: https://theautomatic.net/yahoo_fin-documentation/
        elif data_source == "yahoo_fin":
            # yahoo_finance的國家代碼間隔號使用‘.’，股類分隔號使用'-'，如：rds-b，但資料夾內部存檔時一律用‘_’
            if country == "US":
                ticker_for_search = ticker.replace('.', '-')
            ...
            ticker_for_saveFile = ticker.replace('.', '_')
            file_position = os.path.join(self.database_folder_path, folder_name, ticker_for_saveFile+".pkl")
            price_data = _download_data_from_Yahoo_Fin(ticker_for_search)
            price_data.to_pickle(file_position)
        else:
    ...
    def save_stockPrice_to_pkl(self, ticker_list, country="US", data_source="yfinance"):
        folder_name = self.stock_folder_name
        threads = []  # 儲存線程以待關閉
        num_completed = 0
        num_uncompleted = 0
